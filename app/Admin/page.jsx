...
<p className="md:w-[30%] hidden md:block  cursor-default text-sm font-bold  text-zinc-700 ml-2">Phone number</p>
      ...
      {accounts .filter((account) => account.email !== 'admin@com').map((account, index) => (
        <div key={index}    className="w-full cursor-pointer   rounded-lg hover:bg-blue-200 hover:scale-[101%] group/item mb-1 px-4 mx-4 relative  mt-1 items-center flex ">
            ...
            <div className="absolute gap-2  invisible md:group-hover/item:visible flex justify-evenly top-1/2 -translate-y-1/2 right-[5%]  h-[50%] aspect-[2/1]">
            <Link className="z-50" href={`/Admin/Employee?id=${encodeURIComponent(account.email)}`}><AiFillEdit className="w-6 hover:scale-150 cursor-pointer h-6 text-green-500"/> </Link>
